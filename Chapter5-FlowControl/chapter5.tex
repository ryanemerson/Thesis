...
%    \subsubsection*{Leaky Bucket Extension}
%    So far we have only considered a solution that manages application requests on a per message basis.  A common technique utilised in flow control protocols is the \emph{leaky-bucket} algorithm, which 
    
    % Introduce the concept of buckets
    % Messages added to bucket
    % If a message fills the bucket, then that bucket is ready to send
    % Subsequent calls to add messages are blocked until the full bucket has been broadcast to abcast
    % Process restarts itself as soon as the past bucket has been sent
    % Delay between buckets ensures the broadcast rate
\section{Limitations}
