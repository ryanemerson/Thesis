...
%            Note that $x (x + 1) > 3x \quad \forall x > 2$, i.e. when $|m.dst| > 3$.  
	        ...
	        Finally, the TOA implementation used by Infinispan does not guarantee \emph{uniform agreement} ($\S$ \ref{sec:atomic_guarantees}) when a message originator crashes.  Consider the scenario shown in Figure \ref{fig:TOA}. If the coordinator $N_1$ fails after sending $deliver(m)$ to $N_2$, but before sending the same message to $N_3$, then $N_2$ delivers $m$ but $N_3$ cannot as it never received $deliver(m)$ from $N_1$.
