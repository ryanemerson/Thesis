...
%        The delivery delay, $\Delta_m$ utilised by \textsf{Aramis} is calculated based upon the values utilised by \emph{rbcast} and calculated by the DMC.  $\Delta_m$ is calculated as follows:
%        
%        \begin{equation*}
%		    \begin{aligned}
...
%		
		...
		\subsubsection*{Rejecting Missing Messages}\label{ssec:abcast_rejection}
		    % Conditions required for a message to be rejected!
			% Messages rejected if a later message has already been delivered.
			% Throw exception.
		
    \subsection{\textsf{ABcast} Delivery Rule}
