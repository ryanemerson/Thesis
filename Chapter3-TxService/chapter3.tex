...
Figure \ref{fig:abaas_concept} shows the three key stages in the \textsf{AbaaS} model.  Stage 1 involves the $Tx.c$ requesting an ordering for $Tx$, $r(Tx)$,  from the ordering service, and occurs once the $Tx.c$ has executed locally and is ready to broadcast $Tx$ to all $d \in Tx.dst$.  Upon receiving $r(Tx)$, the ordering service updates the records of all $s$-nodes, and sends an ordering value back to $Tx.c$ (stage 2), whom upon receiving the ordering, broadcasts $Tx.c$ to all $d \in Tx.dst$ (stage 3).  
