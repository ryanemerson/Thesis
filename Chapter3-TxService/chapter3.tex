...
We have developed a protocol for the \textsf{AbaaS} model, called \textsf{Dcup} as from the $c$-nodes perspective it decouples \emph{abcast} ordering and broadcast.  \textsf{Dcup} satisfies all of the requirements specified in $\S$ \ref{sec:absaas_requirements} with the exception of S2.  S2 cannot be satisfied by our protocol directly, rather \textsf{Dcup} relies on a \emph{abcast} protocol to guarantee S1, S3 and S4, therefore in order for S2 to be satisfied, the underlying \emph{abcast} protocol must not block in the presence of node failures.  

\textsf{Dcup} consists of six distinct phases, each of which will is explored in detail in their order of execution below.  Note, a collection of $s$-nodes providing \emph{abcast} ordering are collectively referred to as the \emph{ordering service}.  

	\begin{description}
		\item[Client Request - Client] \hfill \\
		Once a $Tx_i.c$ has completed its local execution of $Tx_i$, it is ready to \emph{abcast} a $prepare(Tx_i)$ message as required by the total order commit protocol.  $Tx_i.c$ randomly selects a $s$-node, $N_s$, from the \emph{service}, and sends an ordering request $order(Tx_i)$ message $m$ to $N_s$, $m.snode = N_s$, with $m_i.dst = Tx_i.dst$.  
		...
		\item[Receive Request - Ordering Service] \hfill \\
		Upon receiving $order(Tx_i)$, the \emph{ordering service} places the request in the \emph{Abcast Request Pool} (ARP), which is a bounded queue used for storing requests before they are processed by the $s$-node.  If the ARP becomes full, $c$-node requests are rejected by the service and a \emph{reject} response is sent to $Tx.c$ whom can abort $Tx$ or resend $order(Tx_i)$ after a configurable amount of time.  
		
		The ARP is necessary to ensure that if the \emph{ordering service} starts to become overloaded by client requests their is a 'push back' mechanism that makes clients aware of the services current limitations, allowing clients to restrict user operations if necessary.  An ARP is also essential for providing message bundling, which as described in \ref{ssec:bundling} is an effective optimisation for improving the throughput of the \emph{ordering service}.  
		
		\item[Process ARP - Ordering Service] \hfill \\
		A single thread, \emph{send} thread, is utilised for retrieving requests from the ARP and \emph{abcast}ing them to all $s$-nodes for ordering.  If the ARP is empty, then the \emph{send} thread waits for the ARP to become non-empty before resuming \emph{abcast}ing.  When message bundling is enabled the \emph{send} thread retrieves requests from the ARP in their arrival order, and bundles them into a single \emph{abcast} message $mb$; message bundles have a configurable upper limit on the maximum size of the bundle message, if this upper limit is reached and the ARP still has available requests, the \emph{send} thread will \emph{abcast} the next message bundle once $mb$ has been \emph{abcast}.  
		
		All \emph{abcast} messages $m$, or message bundles $mb$, sent by a $s$-node $N_s$ have their originator set to the sending $s$-node, $m.o$, $mb.o$ = $N_s$.  This is necessary for the next phase of the protocol.  
		
		\item[Receive Ordering]
		When a $s$-node, $N_s$, receives an \emph{abcast} bundle $mb$, it 'un-bundles' $mb$ and processes each request $r$ in the order that they arrived in the ARP at $mb.o$.  If $N_s$ is the $s$-node that initial received the $r$, then $N_s$ must send $r$ back to $Tx.c$ with two types of ordering data: $m.ts$ agreed by $s$-nodes for $r$ and the \emph{immediate} predecessor.  The latter is the identity of $ms.ts'$ whose delivery must \emph{precede} \emph{immediately} before delivery of $m.ts$.  
		
		The storage of \emph{immediate} predecessor data works as follows: All $s$-nodes maintain a map that stores a transaction history by mapping a $c$-node address with the id of the last transaction they were associated with, i.e. the last ordering request received that contained $c$-node in $m.dst$, hence \emph{immediate} predecessor.  So for each client request the associated $m.ts$ is stored in this map for each $d \in m.dst$. When a $s$-node receives an \emph{abcast} bundle $mb$, it knows that all other nodes have received, or will receive, $mb$ in the same order.  Therefore, when $mb$ is processed by an $s$-node, it is guaranteed that all other $s$-nodes will have processed $mb$ in the exact same order, therefore we know that the transaction history will be consistent for all $s$-nodes.  
