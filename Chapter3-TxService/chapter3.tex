...
Figure \ref{fig:abaas_concept} shows the two key stages in the \textsf{AmaaS} model from the perspective of a $c$-node.  Stage 1 involves the $Tx.c$ requesting a multicast for $Tx$, $req(Tx)$,  from the multicast service, and occurs once the $Tx.c$ has been executed locally and is ready for multicast to each destination $d \in Tx.dst$; $req(Tx)$ contains the $Tx$ and all associated data, such as $Tx.dst$.  Upon receiving $req(Tx)$, the multicast service updates the records of all $s$-nodes and multicasts $Tx$, with a total order value determined by the service, to all $d \in Tx.dst$ (Stage 2).  
...
\section{SCast: A Fault-tolerant Atomic Multicast Protocol}\label{sec:scast_protocol}
