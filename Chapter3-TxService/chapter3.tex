...
In the explanations below we assume that Infinispan is executing a 1-Phase Total Order transaction, without a second WSC phase, and that the transaction has already been successfully executed locally.  Furthermore, we assume that a reliable network protocol is being utilised as the underlying communication mechanism, for example TCP\citep{Cerf:2005:PPN:1064413.1064423} or Reliable UDP\citep{ReliableUDP}.  Finally, we refer to a collection of $s$-nodes providing the \emph{amcast} service as the \emph{multicast service}.  
   ...
   \subsection*{Client Nodes}
    ...
    \begin{description}
        ...
        The purpose of creating a backup transaction coordinator is to allow for the possibility that the original $Tx.c$ may crash during the multicast process, in which case the group membership protocol will detect the crash and the backup coordinator will assume responsibility for multicasting the $prepare(Tx_i)$ message.  Depending on the level of fault-tolerance required, it is possible for the new coordinator to revert back to stage 1 or 2 of the protocol, the benefits of each are explored in \ref{ssec:scast_fault_tolerance}.  
	...
	\subsection{Fault-Tolerance}\label{ssec:scast_fault_tolerance}
	...
	\begin{description}
         ...
         If a $c$-node, $Tx.c$, crashes during or directly after the local execution of a transaction, $Tx_i$, then no action needs to be taken fas no interactions with other nodes has occurred.  
		...
		If $Tx.c$ crashes during the creation of a backup coordinator then two scenarios are possible:
		    \begin{itemize}
			    ...
			    \item    The backup coordinator successfully receives the $prepare(Tx_i)$ message and attempts to ack the original coordinator.  In which case the original coordinator will never receive the ack, and the backup coordinator will restart the multicasting process. 
		    \end{itemize}     
		    
		\item[\emph{Stage 2}]  \hfill \\
        If $Tx.c$ crashes before, or during, the sending of a request to the multicast service, then the backup coordinator will simply restart the multicast process when the group membership service recognises that $Tx.c$ has crashed.  It is possible that the $s$-node that the request was sent to will still see receive the request, in which case the request will be processed as normal by the $s$-node.  This means that when the backup coordinator sends a new multicast request to the service, the service will accept the original request from $Tx.c$.  Therefore if a $s$-node receives a request that is from a backup coordinator, not the original $Tx.c$, the $s$-node will record the address of the backup coordinator locally and send a response to the backup coordinator when the original request arrives via \emph{abcast}.  Of course it is possible that the original request was never received, in which case the $s$-node that received the request from the backup coordinator will simply send a response message when the backup coordinators request has been delivered via \emph{abcast}.  
        
        \item[\emph{Stage 3}]  \hfill \\
        If $Tx.c$ crashes before receiving a response from the multicast service, then the backup coordinator will take over and restart the process.  When the backup coordinators request is received from the service, the service checks its recent history of processed requests and simply returns the ordering data for the requested transaction to the backup coordinator.  The size of the recent history stored by the service should be configurable, however as the size of the past history increases the chances of a prior transaction ordering being discarded decreases, therefore a larger record is advantageous for fault-tolerance.  
...
To test our assumption that the \textsf{AmaaS} model would improve transaction performance, we developed an experiment that utilises the \textsf{SCast} Protocol (\ref{sec:decoupled_protocol}).  The details of these experiments are outlined below. 
